...
public class AnswersXmlImport {
    ...
    private void importMultipleChoiceXmlImport(Element questionAnswerElement, QuestionAnswer questionAnswer) {
        ...
        questionAnswerElement.getChildren(OPTION).forEach(opt -> {
            ...
            optionsIds.add(multipleChoiceQuestionMap.get(questionKey).get(optionSequence));
        });
        ...
        if (optionsIds.isEmpty()) {
            ...
            options.sort(Comparator.comparing(opt -> optionsIds.indexOf(opt.getId())));

            MultipleChoiceAnswer answer
                    = new MultipleChoiceAnswer(questionAnswer, options);
            questionAnswer.setAnswerDetails(answer);
            answerDetailsRepository.save(answer);
        }
