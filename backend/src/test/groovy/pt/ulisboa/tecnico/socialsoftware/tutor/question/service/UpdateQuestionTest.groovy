...
class UpdateQuestionTest extends SpockTest {
    ...
    def "update a question"() {
        ...
        multipleChoiceQuestionDto.setAnswerType(questionDto.getQuestionDetailsDto().getAnswerType())
    ...
    def "update question with two options true in SINGLE correct answer"() {
        given: 'a question'
        def questionDto = new QuestionDto(question)
        def multipleChoiceQuestionDto = new MultipleChoiceQuestionDto()
        multipleChoiceQuestionDto.setAnswerType(questionDto.getQuestionDetailsDto().getAnswerType())
    ...
    def "update correct option in a question with answers"() {
        ...
        def answerDetails = new MultipleChoiceAnswer(questionAnswer, optionsList)
        ...
        optionsList.add(optionKO)

        questionAnswer = new QuestionAnswer()
        answerDetails = new MultipleChoiceAnswer(questionAnswer, optionsList)
        questionAnswer.setAnswerDetails(answerDetails)
        questionAnswer.setQuizQuestion(quizQuestion)
        questionAnswer.setQuizAnswer(quizAnswer)
        questionAnswerRepository.save(questionAnswer)
    ...
    def "update question from SINGLE to MULTIPLE correct answers"(){
        given: 'a question'
        def questionDto = new QuestionDto(question)
        questionDto.setQuestionDetailsDto(new MultipleChoiceQuestionDto())
        ...
        and: 'and the new set of options is in the question'
        result.getQuestionDetails().getOptions().size() == 3
        def resOptionIncorrect = result.getQuestionDetails().getOptions().stream().filter({ option -> !option.isCorrect()}).findAny().orElse(null)
        resOptionIncorrect != null
        def resOptionCorrects = result.getQuestionDetails().getOptions().stream().filter({ option -> option.isCorrect()}).findAll()
        resOptionCorrects.size() == 2
    }

    def "update question from SINGLE to MULTIPLE with one correct answer"(){
        given: 'a question'
        def questionDto = new QuestionDto(question)
        questionDto.setQuestionDetailsDto(new MultipleChoiceQuestionDto())
