...
class ConcludeQuizTest extends SpockTest {
    ...
    def 'conclude quiz without conclusionDate, without answering'() {
        ...
        correctAnswerDto.getCorrectAnswerDetails().getCorrectOptionsIds().get(0) == optionOk.getId()
    }
    ...
    def 'conclude quiz with multiple options selected, before conclusionDate'() {
        given: 'a quiz with future conclusionDate'
        quiz.setConclusionDate(DateHandler.now().plusDays(2))
        and: 'an answer'
        def statementQuizDto = new StatementQuizDto()
        ...
        def optionsResult = ((MultipleChoiceAnswer) questionAnswer.getAnswerDetails()).getOptions()
        optionsResult.get(0) == optionOk
        optionOk.getQuestionAnswers().contains(questionAnswer.getAnswerDetails())
        optionsResult.get(1) == optionC
        optionC.getQuestionAnswers().contains(questionAnswer.getAnswerDetails())

        and: 'the return value is OK'
        correctAnswers.size() == 1
        def correctAnswerDto = correctAnswers.get(0)
        correctAnswerDto.getSequence() == 0
        correctAnswerDto.getCorrectAnswerDetails().getCorrectOptionsIds().contains(optionOk.getId())
        correctAnswerDto.getCorrectAnswerDetails().getCorrectOptionsIds().contains(optionC.getId())
    }

    def 'conclude quiz with multiple ordered options selected, before conclusionDate'() {
        given: 'a quiz with future conclusionDate'
        quiz.setConclusionDate(DateHandler.now().plusDays(2))
        and: 'an answer'
        def statementQuizDto = new StatementQuizDto()
        ...
        def optionsResult = ((MultipleChoiceAnswer) questionAnswer.getAnswerDetails()).getOptions()
        optionsResult.get(0) == optionRepository.findById(optionsResult.get(0).getId()).get()
        optionA.getQuestionAnswers().contains(questionAnswer.getAnswerDetails())
        optionsResult.get(1) == optionRepository.findById(optionsResult.get(1).getId()).get()
        optionB.getQuestionAnswers().contains(questionAnswer.getAnswerDetails())
        
        and: 'the return value is OK'
        correctAnswers.size() == 1
        def correctAnswerDto = correctAnswers.get(0)
        correctAnswerDto.getSequence() == 0
        correctAnswerDto.getCorrectAnswerDetails().getCorrectOptionsIds().contains(optionA.getId())
        correctAnswerDto.getCorrectAnswerDetails().getCorrectOptionsIds().contains(optionB.getId())
    }
