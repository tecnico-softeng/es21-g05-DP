...
class SpockTest extends Specification {
    ...
    public static final String USER_1_PASSWORD = "1234"
    public static final String USER_2_PASSWORD = "4321"
    ...
    public static final String QUESTION_4_REGEX = "Question 4 Content\n ![image][image]\n question content"
    ...
    public static final String OPEN_ENDED_ANSWER_1 = "This is an open ended answer"
    ...
    public static final Integer ITEM_1_COMBINATION = 0
    public static final Integer ITEM_2_COMBINATION = 1
    public static final Integer ITEM_3_COMBINATION = 2
    public static final Integer ITEM_4_COMBINATION = 3
    public static final Integer ITEM_5_COMBINATION = 4

    public static final String ROLE_STUDENT = "ROLE_STUDENT"
    public static final String ROLE_DEMO_STUDENT = "ROLE_DEMO_STUDENT"
    public static final String ROLE_TEACHER = "ROLE_TEACHER"
    public static final String ROLE_DEMO_TEACHER = "ROLE_DEMO_TEACHER"
    ...
    @Autowired
    CombinationItemRepository combinationItemRepository
    ...
    static CombinationItem createItem(String content, List<Integer> combinations, CombinationItem.Group group){
        def item = new CombinationItem()
        item.setContent(content)
        item.setGroup(group)
        for(Integer comb : combinations){
            item.addCombination(comb)
        }
        return item
    }

    static Long countGroupItems(List<CombinationItemDto> items, String group) {
        return items.stream().filter(op-> op.getGroup().equals(group)).count()
    }

    static void connectItemDtos(CombinationItemDto item1, CombinationItemDto item2) {
        item1.addConnection(item2)
    }

    static void connectItems(CombinationItem item1, CombinationItem item2) {
        item1.addConnection(item2)
    }

    static QuestionDto createItemCombinationQuestion() {
    ...
    static QuestionDto createSingleAnswerMultipleChoiceQuestion() {
        def questionDto = new QuestionDto()
        questionDto.setTitle(QUESTION_1_TITLE)
        questionDto.setContent(QUESTION_1_CONTENT)
        questionDto.setKey(1)
        ...
        multipleChoiceQuestionDto.setAnswerType(MultipleChoiceQuestion.AnswerType.SINGLE.name())
        multipleChoiceQuestionDto.setOptions(options)
        questionDto.setQuestionDetailsDto(multipleChoiceQuestionDto)
        return questionDto
    }

    static QuestionDto createMultipleAnswerMultipleChoiceQuestion() {
        def questionDto = new QuestionDto()
        ...
        def multipleChoiceQuestionDto = new MultipleChoiceQuestionDto()
        multipleChoiceQuestionDto.setAnswerType(MultipleChoiceQuestion.AnswerType.MULTIPLE.name())
        multipleChoiceQuestionDto.setOptions(options)
        questionDto.setQuestionDetailsDto(multipleChoiceQuestionDto)
        return questionDto
    }

     static QuestionDto createOrderedAnswerMultipleChoiceQuestion() {
        def questionDto = new QuestionDto()
        ...
        def multipleChoiceQuestionDto = new MultipleChoiceQuestionDto()
        multipleChoiceQuestionDto.setAnswerType(MultipleChoiceQuestion.AnswerType.ORDER.name())
        multipleChoiceQuestionDto.setOptions(options)
        questionDto.setQuestionDetailsDto(multipleChoiceQuestionDto)
        return questionDto
    }

    def loginAsADemo(String demo) {
        switch (demo) {
            case ROLE_DEMO_TEACHER:
                demoTeacherLogin()
                break
            case ROLE_DEMO_ADMIN:
                demoAdminLogin()
                break
            case ROLE_DEMO_STUDENT:
                demoStudentLogin()
                break
        }
    }
