...
Cypress.Commands.add('beforeEachTournament', () => {
  dbCommand(`
      WITH tmpCourse as (SELECT ce.course_id, ce.id as course_execution_id FROM courses c JOIN course_executions ce on ce.course_id = c.id WHERE name = 'Demo Course')      
        ...
        ,insert10 as (INSERT INTO question_details (id, question_type, question_id, answer_type) VALUES (1000, 'multiple_choice', 1389, 'SINGLE'))
  ...
  for (let content in [0, 1, 2, 3]) {
    ...
    const order = 0;
    dbCommand(`
      INSERT INTO options(content, correct, question_details_id, sequence, correct_order) VALUES ('${content}', '${correct}', 1000, ${content}, ${order});
      `);
  }
});
...
Cypress.Commands.add('addQuestionSubmission', (title, submissionStatus) => {
  ...
  for (let content in [0, 1, 2, 3]) {
    ...
    dbCommand(
      ...
      quest_details as (INSERT INTO question_details (question_type, question_id, answer_type) VALUES ('multiple_choice', (SELECT id FROM quest), 'SINGLE') RETURNING id)
      INSERT INTO options(content, correct, question_details_id, sequence, correct_order) 
      VALUES ('${content}', '${correct}', (SELECT id FROM quest_details), ${content}, ${order});`
    );
  }
});

Cypress.Commands.add('removeMultipleChoiceQuizOptions', (quizTitle) => {
  dbCommand(`
    WITH q_id AS (SELECT id FROM quizzes WHERE title like '%${quizTitle}%')
    , quizQ_id AS (SELECT id FROM quiz_questions WHERE quiz_id IN (SELECT * FROM q_id))
    , ansD_id AS (SELECT id FROM answer_details WHERE question_answer_id IN (SELECT id FROM question_answers WHERE quiz_question_id IN (SELECT * FROM quizQ_id)))
    DELETE FROM options_id WHERE question_answers_id IN (SELECT * FROM ansD_id);
  `);
});

Cypress.Commands.add( 'removeQuizAnswersAndQuiz', (quizTitle) => {
  dbCommand(`
    WITH q_id AS (SELECT id FROM quizzes WHERE title like '%${quizTitle}%')
    , quizQ_id AS (SELECT id FROM quiz_questions WHERE quiz_id IN (SELECT * FROM q_id))
    , ansD AS (DELETE FROM answer_details WHERE question_answer_id IN (SELECT id FROM question_answers WHERE quiz_question_id IN (SELECT * FROM quizQ_id)))
    , quA AS (DELETE FROM question_answers WHERE quiz_question_id IN (SELECT * FROM quizQ_id))
    , quizQ AS (DELETE FROM quiz_questions WHERE id IN (SELECT * FROM quizQ_id))
    , quizA AS (DELETE FROM quiz_answers WHERE quiz_id IN (SELECT * FROM q_id))
    DELETE FROM quizzes WHERE quizzes.id IN (SELECT * FROM q_id);
  `);
});
...
Cypress.Commands.add('cleanItemCombinationQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_combination' where title like '%${questionName}%')
                  , auxqd AS (SELECT * FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id)
                  , conn AS (DELETE FROM connections WHERE connections_id IN (SELECT connections.connections_id FROM auxqd))
                  , item AS (DELETE FROM combination_item WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                  , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});
